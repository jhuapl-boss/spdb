...
class ImageCube8(Cube):
    ...
    def xy_image(self, z_index=0):
        ...
        return Image.frombuffer('L', (x_dim, y_dim), self.data[0, :, :].flatten(), 'raw', 'L', 0, 1)
    ...
    def xz_image(self, z_scale=1, y_index=0):
        ...
        """
        z_dim, y_dim, x_dim = self.data.shape
        out_image = Image.frombuffer('L', (x_dim, z_dim), self.data[:, y_index, :].flatten(), 'raw', 'L', 0, 1)
...
class ImageCube16(Cube):
    ...
    def xy_image(self, z_index=0):
        ...
        if self.data.dtype == np.uint8:
            return Image.frombuffer('L', (x_dim, y_dim), self.data[z_index, :, :].flatten(), 'raw', 'L', 0, 1)
        else:
            out_image = Image.frombuffer('I;16', (x_dim, y_dim), self.data[z_index, :, :].flatten(), 'raw', 'I;16', 0, 1)
    ...
    def xz_image(self, z_scale=1, y_index=0):
        ...
        if self.data.dtype == np.uint8:
            ...
            out_image = Image.frombuffer('I;16', (x_dim, z_dim), self.data[:, y_index, :].flatten(), 'raw', 'I;16', 0, 1)
    ...
    def yz_image(self, z_scale=1, x_index=0):
        ...
        if self.data.dtype == np.uint8:
            ...
            out_image = Image.frombuffer('I;16', (y_dim, z_dim), self.data[:, :, x_index].flatten(), 'raw', 'I;16', 0, 1)
            out_image = out_image.point(lambda i: i * (1. / 256)).convert('L')
