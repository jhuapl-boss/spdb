...
class BossResourceDjango(BossResource):
    ...
    def populate_collection(self):
        """
        Method to create a Collection instance and set self.__collection.
        """
        self.__collection = Collection(self.boss_request.collection.name,
                                       self.boss_request.collection.description)

    def populate_coord_frame(self):
        """
        Method to create a CoordinateFrame instance and set self.__coord_frame.
        """
        self.__coord_frame = CoordinateFrame(self.boss_request.coord_frame.name,
    ...
    def populate_experiment(self):
        """
        Method to create a Experiment instance and set self.__experiment.
        """
        self.__experiment = Experiment(self.boss_request.experiment.name,
    ...
    def populate_channel_or_layer(self):
        """
        Method to create a Channel or Layer instance and set self.__channel or self.__layer.
        """
        if self.boss_request.channel_layer.is_channel:
            # You have a channel request
            self.__channel = Channel(self.boss_request.channel_layer.name,
            ...
            self.__layer = Layer(self.boss_request.channel_layer.name,
                                 self.boss_request.channel_layer.description,
                                 self.boss_request.channel_layer.datatype,
                                 self.boss_request.channel_layer.max_time_step,
    ...
    def populate_time_samples(self):
        """
        Method to set self.__time_samples.
        """
        # TODO: add time sample support
        self.__time_samples = [0]

    def populate_boss_key(self):
        """
        Method to set self.__boss_key.
        """
        self.__boss_key = self.boss_request.get_bosskey
